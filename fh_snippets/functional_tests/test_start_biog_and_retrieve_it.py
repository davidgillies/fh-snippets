...
class NewVisitorTest(FunctionalTest):
    ...
    def setUp(self):
        ...
        self.browser.implicitly_wait(3)  # gives FF response time
    ...
    def test_dashboard_in_title(self):
        ...
        # self.fail('Finished')
        # in to make sure test fails til you finished it.
    ...
    def test_can_start_a_biog_and_retrieve_it_later(self):
        ...
        first_name_inputbox = self.browser.find_element_by_id(
            'new_biog_first_name')
        self.assertEqual(
            first_name_inputbox.get_attribute('placeholder'),
            'Enter a First Name'
        )
        surname_inputbox = self.browser.find_element_by_id(
            'new_biog_surname')
        self.assertEqual(
            surname_inputbox.get_attribute('placeholder'),
            'Enter a Surname'
        )
        birth_year_inputbox = self.browser.find_element_by_id(
            'new_biog_birth_year')
        self.assertEqual(
